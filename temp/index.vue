...
let tours = await useFetch(
  `http://127.0.0.1:8000/api/filtered-tours?d=${JSON.stringify(
    filter.value.destinations
  )}&c=${JSON.stringify(filter.value.categories)}&page=${filter.value.page}`
).data;

const countries = await useFetch(
...
const getFilteredTours = async (filter) => {
  let { data: tours } = await useFetch(
    `/api/filtered-tours?d=${JSON.stringify(
      filter.destinations
    )}&c=${JSON.stringify(filter.categories)}&page=${filter.page}`
  );
};
