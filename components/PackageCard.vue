<template>
  ...
  >
    <div class="relative">
      <img
        :src="`${config.public.BASE_URL}/images/packages/${product.thumbnail}`"
        alt="image"
    ...
    <div class="h-72 relative">
      <div class="items-center border-b mb-2 h-16 flex px-2">
        <h3 class="text-lg text-black font-bold">
          {{
            product["title_" + locale].substring(0, 50) +
            (product["title_" + locale].length > 50 ? "..." : "")
          }}
        </h3>
      </div>
      <div
      ...
      >
        <div class="grid gap-0 grid-cols-2" v-if="product['banner_highlights']">
          <div
            v-for="(highlight, i) in JSON.parse(product['banner_highlights'])"
            :key="highlight"
            class="flex h-12"
          >
